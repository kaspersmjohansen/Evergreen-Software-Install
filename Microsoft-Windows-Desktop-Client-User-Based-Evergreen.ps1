...
<#
*************************************************************************************************
Name:               Microsoft-Remote-Desktop-User-Based-Evergreen
Author:             Kasper Johansen
Website:            https://virtualwarlock.net            

*************************************************************************************************
.DESCRIPTION
    This script installs the latest Microsoft-Remote-Desktop-Evergreen 
...
$Vendor = "Microsoft"
$Product = "Windows Desktop Client"
$EvergreenApp = Get-EvergreenApp -Name MicrosoftWvdRemoteDesktop | Where-Object {$_.Architecture -eq "x64" -and $_.Channel -eq "Public"}
...
$EvergreenAppFile = $EvergreenApp.filename
...
$InstallArguments = "REBOOT=ReallySuppress /qn ALLUSERS=2 MSIINSTALLPERUSER=1"
...
Write-Host ""
Invoke-WebRequest -UseBasicParsing -Uri $EvergreenAppURL -OutFile $Destination\$EvergreenAppFile

Write-Host "Installing $Vendor $Product v$EvergreenAppVersion" -ForegroundColor Cyan
Write-Host ""
Start-Process -FilePath $Destination\$EvergreenAppFile -Wait -ArgumentList $InstallArguments